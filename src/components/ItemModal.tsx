  ...
  return (
    <div className="fixed inset-0 z-50 bg-white overflow-auto p-4">
      <div className="max-w-2xl mx-auto">
        <button onClick={onClose} className="text-sm text-gray-600 mb-2">Close</button>
        <h2 className="text-2xl font-bold">{item.name}</h2>
        <p className="text-gray-600">{item.description}</p>

        <form onSubmit={handleSubmit(onSubmit)} className="mt-4">
          <fieldset>
            <legend className="font-semibold mb-2">Variant</legend>
            {item.variants.map(v => (
            ...
            {errors.variantId && <div className="text-red-500 text-sm">Please choose a variant</div>}
          </fieldset>
          ...
          <label className="block mt-3">Notes
            <textarea {...register('notes')} className="w-full p-2 border rounded mt-1" rows={2} />
          </label>

          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t">
            <button type="submit" className="w-full bg-orange-500 text-white p-3 rounded font-bold">Add ({formatCurrency((item.variants[0]?.price || 0) * quantity)})</button>
          </div>
        </form>
      </div>
    </div>
