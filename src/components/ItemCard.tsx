...
export const ItemCard: React.FC<{ item: MenuItem; onSelect: () => void }> = ({ item, onSelect }) => {
  ...
  return (
    <article className="bg-white rounded-md shadow-sm overflow-hidden cursor-pointer hover:shadow-lg transition-transform transform hover:-translate-y-1" onClick={onSelect}>
      ...
      <div className="p-2">
        <div className="font-semibold text-sm sm:text-base truncate text-primary">{item.name}</div>
        {item.description && <div className="text-xs text-gray-600 mt-1 line-clamp-2">{item.description}</div>}
        <div className="mt-2 text-secondary font-semibold text-sm">{formatCurrency(price)}</div>
      </div>
    </article>
  )
}
