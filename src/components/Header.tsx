    ...
    <header className="bg-white border-b p-3 sticky top-0 z-20">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-3">
          {onBack && (
            <button aria-label="Kembali" onClick={onBack} className="w-10 h-10 rounded-sm bg-orange-500 text-white flex items-center justify-center">
          ...
          {onToggleCategories && (
            <button aria-label="Toggle categories" onClick={onToggleCategories} className="md:hidden w-10 h-10 rounded-sm bg-orange-500 text-white flex items-center justify-center">
        ...
        <div className="flex items-center space-x-3">
          <Link to="/cart" className="text-sm">Keranjang</Link>
          <div className="text-sm font-semibold">{totalCount} • {formatCurrency(summary.total || 0)}</div>
        </div>
      </div>
    </header>
  )
